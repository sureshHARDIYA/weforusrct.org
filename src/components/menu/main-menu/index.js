        ...
        <MainMenuWrap {...props}>  
            <NavBar>
                {menuarr.map(menu => { 
                    ...
                    return (
                        <NavItem key={`mainmenu-${menu.node.id}`} hasSubmenu={hasSubmenu} hasMegamenu={hasMegamenu}>
                            <NavLink path={menu.node.link} hassubmenu={(hasSubmenu || hasMegamenu) ? "true" : "false"}>
                                <span>{menu.node.text}</span>
                                {(hasSubmenu || hasMegamenu) && <MdKeyboardArrowDown className="icon"/>}
                            </NavLink>
                            {submenu && (
                                <Submenu>
                                                        ...
                                                        {megaSubmenu.map((megaSubitem, i) => {
                                                            return(
                                                                <NavItem key={`megasubmenu-${megaIndex}-${i}`}>
                                                                    <NavLink path={megaSubitem.link}>{megaSubitem.text}</NavLink>
                                                                </NavItem>
                                                            )
                                                        })}
                                                    </Submenu>
